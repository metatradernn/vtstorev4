...
import { CreditCard, Wallet, Landmark, Bitcoin, X, ExternalLink, Loader2, CheckCircle, Copy } from "lucide-react";
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogPortal container={containerRef?.current}>
        ...
        <DialogPrimitive.Content className="absolute left-1/2 top-1/2 z-[101] w-[92%] max-w-[480px] -translate-x-1/2 -translate-y-1/2 border border-zinc-800 bg-black p-6 shadow-2xl rounded-[32px] outline-none">
          ...
          {status === 'pending' && (
            <div className="space-y-4">
              ...
              <button onClick={() => paymentUrl && window.open(paymentUrl, '_blank')} className="w-full text-center text-zinc-500 hover:text-white text-sm transition-colors">
            ...
            <div className="space-y-2 mb-5">
              {MANUAL_METHODS.map((method) => (
                ...
                >
                  ...
                  <button onClick={(e) => { e.stopPropagation(); window.open(method.infoUrl, '_blank'); }} className="p-3 mr-2 rounded-xl bg-white/5 text-zinc-500 hover:text-white hover:bg-white/10 transition-all" title="Инструкция">
              ...
              <div
                ...
                className={`relative flex items-center rounded-2xl border transition-all cursor-pointer ${selectedMethod === 'crypto' ? 'border-white bg-zinc-900' : 'border-transparent bg-zinc-900/50 hover:bg-zinc-800/80'}`}
              >
                <div className="flex-1 flex items-center gap-3 p-4">
                  <span className="text-zinc-400"><Bitcoin className="w-4 h-4" /></span>
                  <div>
                    <span className="font-medium text-[14px] text-zinc-100">Криптовалюта</span>
                    <span className="ml-2 text-[10px] bg-white/10 text-zinc-400 px-2 py-0.5 rounded-full">Автоматически</span>
                  </div>
                </div>
              </div>
